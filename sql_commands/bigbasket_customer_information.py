...
while counter <= 5000:
    ...
    c_fname = random.choice(male_names if random.randint(0,1) == 0 else female_names)
    c_lname = random.choice(last_names)

    random_state_code = random.randint(1, 37)
    random_state_name = states.get(random_state_code)

    random_passwd = f"{c_lname}_{c_fname}@{random.randint(100, 999)}?".encode('utf-8')
    ...
    c_id = round(random.random() * 10**16) # ignore, I am getting the type of number I want here, dont be nosy
    c_name = f'{c_fname} {c_lname}'
    c_email = f'{c_fname}{random.randint(100,999)}{c_lname}@{random.choice(email_domains)}.com'
    c_number = random.randint(6000000000, 9999999999)
    c_address = f"{'...'*70}, {random_state_name}, India"
    hashed_passwd = str(bcrypt.hashpw(random_passwd, bcrypt.gensalt(rounds=6))).replace('b', '').replace('\'', '')
    signup_date = random_date.strftime('%Y-%m-%d %H:%M:%S')

    if c_id in history_of_id:
        continue

    if c_email in history_of_email:
        continue

    if c_number in history_of_number:
        continue

    history_of_id.append(c_id)
...
with ConnectToMySQL('localhost', 'root', 'tks@123?') as conn_ob:
    ...
    for _ in range(0, 5001, 100):
        conn_ob.execute_sql_query(f"{sql_query} {", ".join(map(str, customer_info[_:_+100]))};")
